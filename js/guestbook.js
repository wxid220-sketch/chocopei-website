 ...
 * 双模式：Lời nhắn công khai（明文本地存储） / 私密留言（AES-GCM-256 + PBKDF2，
...
const Guestbook = {
  ...
  async _submit() {
    ...
    try {
      ...
      input.value = '';
      this.render();
      Utils.toast(isPrivate ? 'Lời nhắn已Gửi 🔒' : '留言已Gửi ✨');
      if (isPrivate) Badges.grant('secret');
      Earth.flash(); // 地球表面闪烁反馈（模块联动）
    } catch (e) {
      Utils.toast('Gửi失败', '请重试');
    }
  ...
  /* ---- 解密入口：选中Lời nhắn → Nhập mật khẩu → 验证 authTag ---- */
  async _unlock(msg, el) {
    const pwd = prompt('Nhập mật khẩu解密这条留言 🔒');
    if (!pwd) return;
    try {
    ...
    list.forEach((m, idx) => {
      ...
      block.innerHTML = `
        ...
        <div class="msg-foot">
          ...
          ${isAdmin ? '<button class="del">Xóa</button>' : ''}
        </div>`;
      const textEl = block.querySelector('.msg-text');
      if (m.isEncrypted) {
        textEl.textContent = '▓▓▓▓▓▓▓▓▓▓（加密内容，Cần mật khẩu解密）';
