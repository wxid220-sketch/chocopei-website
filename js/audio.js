/* ============================================================================
 * CyberZenAudioEngine - High Performance Audio Driver
 * Provided by user for low-latency sound playback
 * ==========================================================================*/
class CyberZenAudioEngine {
  constructor() {
    this.ctx = new (window.AudioContext || window.webkitAudioContext)();
    this.buffers = {};
  }

  async registerSound(name, url) {
    try {
      const response = await fetch(url);
      const arrayBuffer = await response.arrayBuffer();
      this.buffers[name] = await this.ctx.decodeAudioData(arrayBuffer);
    } catch (err) {
      console.error('[Audio] Load failed for ' + name + ':', err);
    }
  }
...
const zenAudio = new CyberZenAudioEngine();
zenAudio.registerSound('woodfish', 'assets/sfx-woodfish.mp3');
zenAudio.registerSound('click', 'assets/sfx-click.mp3');
zenAudio.registerSound('bell', 'assets/sfx-bell.mp3');
zenAudio.registerSound('incense', 'assets/sfx-incense.mp3');
zenAudio.registerSound('levelup', 'assets/sfx-levelup.mp3');
zenAudio.registerSound('transition', 'assets/sfx-transition.mp3');
zenAudio.registerSound('peach', 'assets/sfx-peach.mp3');

// Also register BGM tracks
zenAudio.registerSound('bgm-main', 'assets/bgm-main.mp3');
zenAudio.registerSound('bgm-alt', 'assets/bgm-alt.mp3');
