...
const App = {
  ...
  _enterLogin() {
    ...
    const greet = h <= 5 ? '深夜里的灵感，Chocopei 陪您静候黎明。'
      : h <= 18 ? '万物生长，Chocopei 记录您的当下。'
      : 'Sao璀璨，欢迎回到您的私人领域。';
    ...
    const doLogin = async () => {
      ...
      /* 验证：非空 + 长度≤20 + 敏感词过滤 */
      if (!name) { err.textContent = 'Vui lòng nhập tên của bạn'; return this._shake(overlay); }
      if (name.length > 20) { err.textContent = 'Tên tối đa 20 ký tự'; return this._shake(overlay); }
      if (Utils.hasBannedWords(name)) { err.textContent = 'Tên chứa từ nhạy cảm'; return this._shake(overlay); }
  ...
  /* 用户层级视觉：Khách白 / 注册青 / VIP 金 / Quản trị员紫 */
  refreshUserTier() {
    const el = document.getElementById('user-name');
  ...
  _woodfishTap(e) {
    ...
    try { if(typeof zenAudio!="undefined") zenAudio.play("woodfish"); } catch(e) {}
  ...
  renderMurmur() {
    ...
    const list = Utils.get('murmurs', [{ text: 'Chúc bạn một ngày vui vẻ! 💜 🙏', ts: Date.now() }]);
  ...
  _bindEasterEggs() {
    ...
    document.querySelectorAll('.foot-dot').forEach((dot, i) => {
      dot.addEventListener('click', () => {
        ...
        ZXAudio.sfx('click');
        Utils.toast('Đã đổi màu', i === 1 ? '禅意流转 ✨' : '');
      });
    });
  },

  /* Khách感言录：随机 3 条Lời nhắn + 纯净版音乐 */
  _openGuestVoices() {
    ...
    document.getElementById('voices-list').innerHTML = pick.length
      ...
      : '<div class="admin-stat">还没有KháchLời nhắn，去星际终端留下第一条吧 ✨</div>';
  ...
  _quickJump(text) {
    ...
    if (!target) { Utils.toast('未识别的指令', '试试：图片 / 音乐 / Video / Lời nhắn / 足迹 / Quản trị'); return; }
    Utils.click();
    if (target === 'music') document.getElementById('music-panel').classList.add('open');
    else if (target === 'admin') { if (this._isAdmin) Admin.open(); else Utils.toast('需要Quản trị员权限'); }
    else this.showPage(target);
  },

  /* ---- Chia sẻ（7 套场景文案 + Toast） ---- */
  share(scene = 'home') {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(CONFIG.domain).then(() =>
        Utils.toast('链接Đã sao chép', '快去Chia sẻ给懂你的人吧 💫'));
    } else {
  ...
  _bindGlobal() {
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) { document.title = 'Chocopei 正在等待您…'; ZXAudio.onVisibility(true); }
      else { document.title = '欢迎回来 ✦ Chocopei'; ZXAudio.onVisibility(false);
        setTimeout(() => { if (!document.hidden) document.title = 'Chocopei | VinaHouse & Chill'; }, 2500); }
    });
    /* 离线提示 */
    ...
    if ('PerformanceObserver' in window) {
      try {
        new PerformanceObserver(list => {
          ...
          if (e) console.log(`[Chocopei] LCP: ${Math.round(e.startTime)}ms`);
