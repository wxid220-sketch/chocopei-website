/* ============================================================================
 * effects.js — Gửi quà / 支付 / 特效 / 徽章 / 段位
...
const Effects = {
  ...
  handlePayment(platform, amount, onSuccess) {
    ...
    /* 记录待确认đến Chocopei!，用户支付返回后触发成功特效 */
    this._pending = onSuccess || null;
    Utils.toast('即将跳转到支付页', '完成支付后回到本站，点击「✅ Tôi đã thanh toán」领取功德 ✨');
    setTimeout(() => {
  ...
  /* ==================== Cửa hàng quà tặng（面板 3/4/5） ==================== */
  _buildGiftPanel() {
    const grid = document.getElementById('gift-grid');
    const icons = { lotus: '🪷', incense: '🕯️', golden: '🪙', wukong: '👑' };
    /* 默认行为：Gửi quà；热爱板块可临时改写 _giftHandler 复用此面板 */
    ...
    CONFIG.gifts.forEach(g => {
      ...
      card.innerHTML = `<div class="g-icon">${icons[g.effect]}</div>
        ...
        <div class="g-merit">❤️ +${g.merit}</div>`;
  ...
  /* ---- Xác nhận thanh toán面板：选PayPal / Bank/微信 ---- */
  _buildPayPanel() {
  ...
  openPayConfirm(item, onSuccess) {
    ...
    document.getElementById('pay-item-name').textContent = item.name || ('Ủng hộ ' + item.label);
  ...
  openWishForm(tier, onDone) {
    ...
    document.getElementById('wish-tier-label').textContent = `${tier.label} · Ủng hộ ${tier.durationText}`;
    const wishInput = document.getElementById('wish-input');
    const nameInput = document.getElementById('wish-name-input');
...
const Merit = {
  ...
  _checkRank(before, after) {
    ...
    ZXAudio.sfx('levelup');
      ...
      if (ra.name === 'Vương miện 👑') Badges.grant('buddha');
    }
  }
};
